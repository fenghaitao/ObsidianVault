...
- **Context-centric decomposition (not problem-centric)**: split along context boundaries, not work-type boundaries. Splitting by role (one agent plans, one implements, one tests, one reviews) creates a "telephone game" where each handoff loses fidelity — in one experiment, role-specialized subagents spent more tokens coordinating than executing. However, sequential pipelines become viable when handoffs go through **files** rather than conversational summaries: writing results to a shared artifact (e.g., `docs/api-spec.md`) preserves full fidelity that a summary-based handoff would lose, reconciling the apparent tension between the "telephone game" anti-pattern and the April 2026 guidance endorsing sequential pipelines. An agent handling a feature should also handle its own tests, since it already holds the relevant context.
