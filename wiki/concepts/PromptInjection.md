...
- **Computer-use activation scanning (March 2026):** When Claude uses a computer directly in [[ClaudeCowork]] and [[ClaudeCode]] (pointing, clicking, navigating apps/browser/dev tools), Anthropic's system automatically scans internal model activations to detect prompt-injection-style misuse in real time, complementing per-app permission requests and a user override to stop Claude at any point. See [[ComputerUse]].
...
- The March 2026 "Computer-use activation scanning" bullet on this page describes Anthropic "scanning internal model activations to detect prompt-injection-style misuse in real time." The May 2026 best-practices article instead describes "real-time classifiers" that scan content entering the context window across modalities. These may describe the same underlying mechanism in different terms, or two distinct systems (activation-based vs. content-based classifiers) — the May 2026 source does not use the term "activation scanning." Presented as-is rather than silently merged; flagged for future clarification if a source directly reconciles the two framings.
