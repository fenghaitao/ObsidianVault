...
- Three implementation approaches identified by Matt Carey:
  1. **CLI-based**: Agents use shell access to call `--help` and parse commands — used by OpenClaw, requires shell access
  2. **Tool Search**: A search tool loads relevant tools into context via keyword matching — used by Cloud Code, but unused tools remain in context
  3. **Code Mode**: Let the model write code against typed SDKs — the most compact approach, requires sandboxing
- Implementation approaches from David Soria Parra: (1) Use tool search (available in the Anthropic API and competitors' APIs) to defer loading; (2) Build a tool loading tool yourself — give the model a tool that loads other tools, and the model decides when to look up what it needs
...
- [[summary-20260425 - MCP = Mega Context Problem - Matt Carey]] — source (three approaches)
- [[MCP]] — protocol
- [[ProgressiveDisclosure]] — related server-side pattern
- [[ToolSearch]] — implementation mechanism
- [[CLI for Agents]] — CLI-based implementation approach
- [[CodeMode]] — code-based implementation approach
