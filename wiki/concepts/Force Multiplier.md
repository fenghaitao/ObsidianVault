---
title: "Force Multiplier"
type: concept
tags: [career, engineering, impact, leadership]
sources: ["raw/03-transcripts/Ryan L. Peterman/Channel Only/20240708 - Getting To Staff (IC6) at FAANG Panel (Full, Feb 2024).md", "raw/03-transcripts/Ryan L. Peterman/Channel Only/20250523 - OpenAI & Meta Distinguished Engineer (IC9)： Working With Zuck, Carmack & Career Growth ｜ Philip Su.md", "raw/03-transcripts/Ryan L. Peterman/Channel Only/20251111 - Uber Distinguished Eng： Unfair Promos, Influence, Engineering Regrets ｜ Joakim Recht.md"]
last_updated: 2026-09-14
---

## Definition

A Force Multiplier is an approach to work that amplifies the productivity and impact of others, rather than focusing solely on individual output. For Staff Engineers, force multiplying through people leadership or technology leverage is a key way to achieve outsized impact.

## Key Information

- Lee discussed force multiplying as an alternative to working long hours — "if you can make people super super effective then the number of hours you work might not have the same correlation to impact"
- Lee achieved Staff-level impact without working more than 40 hours a week by focusing on force multiplying through people leadership
- Force multiplying through technology (infrastructure, tooling, platforms) is another path — scaling efficiency gains across hundreds of engineers
- The concept contrasts with individual delivery — both can achieve impact, but force multiplying can be more efficient in terms of hours worked
- "The people side can show that impact in less hours spent"
- Zach described infrastructure work as "changing how other engineers are doing their job" — a form of force multiplication

### Scott Renfro as a Force Multiplier
- Philip Su described Scott Renfro as a "slam dunk case" of a force multiplier at Meta
- Scott's peer feedback consistently said: "Scott takes time out of his day to make me great"
- He would review PRs with sensitivity, never publicly shaming anyone, always framing feedback constructively
- Having Scott on a team made the entire team much better
- He was capable both personally of guaranteeing a huge scope of work AND being a force multiplier for teams

### Joakim Recht on the Force Multiplier
- Recht: "you become a forceful multiplier by allowing other people to work better and faster — or maybe not even work at all" on their problem
- The most fundamental form is taking a problem entirely "out of the equation" via tooling, so software acts on your behalf and helps others
- His Odin platform is the example: a single ~20-person team operated all of Uber's stateful workloads, freeing hundreds of engineers from repetitive fleet management

## Related

- [[summary-20240708 - Getting To Staff (IC6) at FAANG Panel (Full, Feb 2024)]] — source summary
- [[Leverage]] — the underlying mechanism of force multiplication
- [[Staff Engineer]] — the level where force multiplying becomes essential
- [[Infrastructure Teams vs Product Teams]] — infrastructure as a force multiplier
- [[Lee]] — panelist who emphasized force multiplying
- [[summary-20250523 - OpenAI & Meta Distinguished Engineer (IC9)： Working With Zuck, Carmack & Career Growth ｜ Philip Su]] — source summary
- [[Scott Renfro]] — described as a force multiplier at Meta
- [[summary-20251111 - Uber Distinguished Eng： Unfair Promos, Influence, Engineering Regrets ｜ Joakim Recht]] — source summary
- [[Joakim Recht]] — force multiplier via Odin
- [[Odin]] — platform as a force multiplier
