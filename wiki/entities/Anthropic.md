...
- **Claude Visualizer**: Uses MCP Apps under the hood for first-party UI delivery. Ruben Casas notes this is strategically significant — Anthropic could have built their own rendering mechanism but chose MCP Apps for its built-in sandboxing, authentication, and message passing
...
- **BullshitBench performance**: Anthropic's Claude models are the top performers on BullshitBench, with Claude 4.5 and Sonnet 4.5 showing dramatic improvement in nonsense detection. Even Haiku performs well. Anthropic is the only major lab showing a clear upward trend in this capability over time.
- **MCP Apps co-development**: Partnered with OpenAI and the MCPUI team to standardize MCP Apps as the first official MCP extension. Claude was the first host to release Claude apps supporting MCP Apps. Anthropic's generative UI feature uses MCP Apps under the hood.
- **Generator-Evaluator Pattern**: Ash Prabaker and Andrew Wilson (Applied AI team) presented Anthropic's internal harness pattern for long-running agents — splitting builder and QA critic into separate context windows with adversarial pressure, contract negotiation before building, and rubric-based evaluation using Playwright. Opus 4.6 runs ~12 hours on minimal scaffold; with this harness, builds fully-featured apps (Retro Forge game maker, DAW music app) over 6+ hours. Key insight: self-evaluation is a trap; adversarial evaluation produces dramatically better results
...
- [[summary-20260424 - What Do Models Still Suck At - Peter Gostev, Arena.ai, BullshitBench]] — source (BullshitBench top performer)
- [[BullshitBench]] — benchmark where Claude models lead
- [[Nonsense Detection]] — capability where Anthropic excels
- [[Peter Gostev]] — BullshitBench creator
- [[summary-20260506 - MCP UI： Extending the frontier — Liad Yosef and Ido Salomon, MCP Apps]] — source (MCP Apps co-development)
- [[summary-20260514 - Ship Real Agents： Hands-On Evals for Agentic Applications — Laurie Voss, Arize]] — source (eval workshop using Claude models)
- [[summary-20260518 - Anthropic Workshop： Build Agents That Run for Hours — Ash Prabaker & Andrew Wilson]] — source (generator-evaluator pattern, long-running agents)
- [[MCP Apps]] — standardized MCP extension co-developed with Anthropic
- [[MCPUI]] — predecessor project that became MCP Apps
- [[GenerativeUI]] — Claude's generative UI feature uses MCP Apps
- **Intercom 2x**: Claude Code is used as the unified AI coding platform at Intercom for the "2x" project. Hundreds of contributors build internal Claude Code plugins. Intercom is also a customer of Finn, Intercom's AI agent.
- [[summary-20260515 - How Building with AI Can Double the Throughput of Your Engineering Team — Brian Scanlan, Intercom]] — source (Intercom's enterprise Claude Code adoption)
- [[Intercom]] — company using Claude Code at scale; also a Finn customer
