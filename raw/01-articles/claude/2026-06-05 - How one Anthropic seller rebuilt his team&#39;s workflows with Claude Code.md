...
`/customer-context `pulls a 360-degree account view across all those sources in about 90 seconds. `/pipeline-management` surfaces at-risk deals, forecasting guidance, and progression recommendations.The package also integrates with Cowork's scheduling feature, which lets reps queue skills to run automatically.
